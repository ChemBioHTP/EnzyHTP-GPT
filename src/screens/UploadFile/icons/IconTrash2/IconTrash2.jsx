/*
We're constantly improving the code you see. 
Please share your feedback here: https://form.asana.com/?k=uvp-HPgd3_hyoXRBw1IcNg&d=1152665201300829
*/

import React from "react";

export const IconTrash2 = ({ className }) => {
  return (
    <svg
      className={`icon-trash-2 ${className}`}
      fill="none"
      height="16"
      viewBox="0 0 16 16"
      width="16"
      xmlns="http://www.w3.org/2000/svg"
    >
      <rect className="rect" fill="white" height="16" style="mix-blend-mode:multiply" width="16" />
      <path className="path" d="M2 4H3.33333H14" fill="#161616" />
      <path className="path" d="M2 4H3.33333H14" stroke="black" strokeLinecap="round" strokeLinejoin="round" />
      <path
        className="path"
        d="M12.6666 3.99992V13.3333C12.6666 13.6869 12.5261 14.026 12.2761 14.2761C12.026 14.5261 11.6869 14.6666 11.3333 14.6666H4.66659C4.31296 14.6666 3.97382 14.5261 3.72378 14.2761C3.47373 14.026 3.33325 13.6869 3.33325 13.3333V3.99992M5.33325 3.99992V2.66659C5.33325 2.31296 5.47373 1.97382 5.72378 1.72378C5.97382 1.47373 6.31296 1.33325 6.66659 1.33325H9.33325C9.68687 1.33325 10.026 1.47373 10.2761 1.72378C10.5261 1.97382 10.6666 2.31296 10.6666 2.66659V3.99992"
        stroke="black"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path className="path" d="M6.66675 7.33325V11.3333" stroke="black" strokeLinecap="round" strokeLinejoin="round" />
      <path className="path" d="M9.33325 7.33325V11.3333" stroke="black" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
};
