/*
We're constantly improving the code you see. 
Please share your feedback here: https://form.asana.com/?k=uvp-HPgd3_hyoXRBw1IcNg&d=1152665201300829
*/

import React from "react";

export const IconFolderPlus = ({ className }) => {
  return (
    <svg
      className={`icon-folder-plus ${className}`}
      fill="none"
      height="16"
      viewBox="0 0 16 16"
      width="16"
      xmlns="http://www.w3.org/2000/svg"
    >
      <rect className="rect" fill="white" height="16" style="mix-blend-mode:multiply" width="16" />
      <path
        className="path"
        d="M14.6666 12.6667C14.6666 13.0203 14.5262 13.3594 14.2761 13.6095C14.0261 13.8595 13.6869 14 13.3333 14H2.66665C2.31302 14 1.97389 13.8595 1.72384 13.6095C1.47379 13.3594 1.33331 13.0203 1.33331 12.6667V3.33333C1.33331 2.97971 1.47379 2.64057 1.72384 2.39052C1.97389 2.14048 2.31302 2 2.66665 2H5.99998L7.33331 4H13.3333C13.6869 4 14.0261 4.14048 14.2761 4.39052C14.5262 4.64057 14.6666 4.97971 14.6666 5.33333V12.6667Z"
        stroke="black"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path className="path" d="M8 7.33334V11.3333" stroke="black" strokeLinecap="round" strokeLinejoin="round" />
      <path className="path" d="M6 9.33334H10" stroke="black" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
};
